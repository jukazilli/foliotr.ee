...
async function assertPresentationOwned(
  tx: TxClient,
  profileId: string,
  ...
  const presentation = await tx.profilePresentation.findFirst({
    where: { id: presentationId, profileId },
    select: { id: true },
  });

  if (!presentation) {
    throw new ApiRouteError("FORBIDDEN", 403, {
      message: "Apresentacao fora do escopo da conta",
    });
  }
}
...
export async function createOwnedVersion(
  ...
  return db.$transaction(async (tx) => {
    ...
    await assertPresentationOwned(tx, profile.id, input.presentationId);
