...
import { archetypeLabels, bigFiveLabels, dimensionLabels, riasecCodes, riasecLabels } from '../types/labels';
import type {
  Answers,
  ArchetypeDimension,
  AreaRecommendation,
...
const bigFiveDimensions: BigFiveDimension[] = ['conscienciosidade', 'amabilidade', 'estabilidadeEmocional', 'extroversao', 'abertura'];
const riasecDimensions: RiasecDimension[] = ['realista', 'investigativo', 'artistico', 'social', 'empreendedor', 'convencional'];
const archetypeDimensions: ArchetypeDimension[] = ['estruturador', 'conectado', 'explorador', 'transformador'];
...
function scoreArea(area: CareerArea, riasec: ScoreMap<RiasecDimension>, bigFive: ScoreMap<BigFiveDimension>, archetype: ScoreMap<ArchetypeDimension>): AreaRecommendation {
  const riasecScore = average(area.riasecFavoravel.map((item) => riasec.normalized[item]));
  const bigFiveScore = average(area.bigFiveFavoravel.map((item) => bigFive.normalized[item]));
  const archetypeScore = average(area.arquetiposFavoraveis.map((item) => archetype.normalized[item]));
  const score = Math.round(riasecScore * 0.55 + bigFiveScore * 0.25 + archetypeScore * 0.2);
...
function getRiasecCode(riasec: ScoreMap<RiasecDimension>): string {
  return riasec.ranking.slice(0, 3).map((item) => riasecCodes[item.key]).join('');
}
...
function calculateConfidence(answers: Answers, riasec: ScoreMap<RiasecDimension>, bigFive: ScoreMap<BigFiveDimension>, archetype: ScoreMap<ArchetypeDimension>): number {
  let confidence = 100;
  ...
  const allHigh = values.length > 0 && values.filter((value) => value >= 4).length / values.length > 0.88;
  const allLow = values.length > 0 && values.filter((value) => value <= 2).length / values.length > 0.88;
  if (allHigh || allLow) confidence -= 15;

  const riasecSpread = riasec.ranking[0].value - riasec.ranking[riasec.ranking.length - 1].value;
  const bigFiveSpread = bigFive.ranking[0].value - bigFive.ranking[bigFive.ranking.length - 1].value;
  const archetypeSpread = archetype.ranking[0].value - archetype.ranking[archetype.ranking.length - 1].value;

  if (riasecSpread < 12) confidence -= 10;
  if (bigFiveSpread < 10 && archetypeSpread < 10) confidence -= 10;

  const nearTies = [riasec.ranking, bigFive.ranking, archetype.ranking].flatMap((ranking) =>
    ranking.slice(0, -1).filter((item, index) => Math.abs(item.value - ranking[index + 1].value) < 5)
  ).length;
...
function buildSummary(profile: UserProfile, riasec: ScoreMap<RiasecDimension>, bigFive: ScoreMap<BigFiveDimension>, archetype: ScoreMap<ArchetypeDimension>, topArea: AreaRecommendation): string {
  const firstName = profile.name?.trim()?.split(' ')[0] || 'Seu resultado';
  const topRiasec = riasec.ranking.slice(0, 3).map((item) => item.label).join(', ');
  const topBigFive = bigFive.ranking.slice(0, 2).map((item) => item.label).join(' e ');
...
function buildStrengths(riasec: ScoreMap<RiasecDimension>, bigFive: ScoreMap<BigFiveDimension>, archetype: ScoreMap<ArchetypeDimension>): StrengthItem[] {
...
function buildAttentionPoints(bigFive: ScoreMap<BigFiveDimension>, riasec: ScoreMap<RiasecDimension>): AttentionItem[] {
...
export function getBlockInstruction(block: Question['block']): string {
  if (block === 'riasec') return 'Responda pensando no quanto cada atividade combina com você, mesmo que você ainda não tenha experiência nela.';
  if (block === 'archetype') return 'Responda pensando no que mais gera satisfação, motivação e senso de propósito para você.';
